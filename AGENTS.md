...
`npm run release` is added with Changesets in the final `chore/release`
milestone, not during the scaffold.
...
### Versioning & changelog — semver via changesets

- While the package remains `0.0.0` and Changesets has not landed, maintain
  `CHANGELOG.md` manually under `[Unreleased]`; never invent a release date or
  version.
- Once the final `chore/release` milestone adds Changesets, every user-visible
  change ships a changeset (`npx changeset`). Pick patch/minor/major and write
  the summary **for the changelog reader**, not the reviewer.
- Mapping: `fix` → patch, `feat` → minor, breaking → major. While on `0.x`,
  changesets' convention applies (breaking → minor, everything else → patch).
- After Changesets lands, released version sections in `CHANGELOG.md` are
  generated by the Version Packages PR and must never be hand-edited. Review
  generated wording against the changesets and README before merging.
- Release flow: changesets opens the "Version Packages" PR → merge → CI publishes
  to npm **with provenance**, tags `vX.Y.Z`, and creates the GitHub Release from
  the changelog entry.
